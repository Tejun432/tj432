...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int j=0; j< wetwet.length; j++){
             ...
             if(wetwet[j][0] == longitude && wetwet[j][1] == latitude){
                 row = j;
                 break;
                }
        ...
        for (int k=0; k< 12; k++){
            wetwetProbability[k] = wetwet[row][k+2];
        }
    }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        for(int i=1; i<forecast.length; i++){
            if(forecast[i-1] == 1){
             if (StdRandom.uniform(0,1) <= wetwetProbability){
                forecast[i] = 1;
            }
            else{
                forecast[i] = 2;
            ...
            if (StdRandom.uniform(0,1) <= drywetProbability){
                forecast[i] = 1;
            }else{
                forecast[i] = 2;
            }
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        double [] drywetProbabilty = new double [12];
        double [] wetwetProbabilty = new double [12];
        populateArrays(drywet, wetwet);
        populateLocationProbabilities(drywetProbabilty, wetwetProbabilty, longitude, latitude, drywet, wetwet);
        return forecastGenerator(drywetProbabilty[month], wetwetProbabilty[month], numberOfDaysInMonth[month]);
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        // COMPLETE THIS METHOD
        int [] count = new int [forecast.length];
        ...
        for(int i=0; i<forecast.length; i++){
           ...
           int longestcount = count[0];
           for (int j=1; j<forecast.length; j++){
               if (count[j]>longestcount){
                   longestcount = count[j];
               }
           }
           return longestcount;
    }
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        // COMPLETE THIS METHOD
        int [] period = new int [forecast.length-6];
